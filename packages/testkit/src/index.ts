...
export async function createMemoryDatabase(): Promise<DeliveryDatabase> {
  ...
  memory.public.none(`
    ...
    CREATE TABLE administrator_invitations (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), email text NOT NULL, email_canonical text GENERATED ALWAYS AS (lower(trim(email))) STORED, target_role text NOT NULL DEFAULT 'ADMIN', token_hash text NOT NULL UNIQUE, status text NOT NULL DEFAULT 'PENDING', idempotency_key text NOT NULL, request_hash text NOT NULL, created_by uuid NOT NULL REFERENCES users(id), accepted_user_id uuid REFERENCES users(id), created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(), expires_at timestamptz NOT NULL, consumed_at timestamptz, revoked_at timestamptz, UNIQUE(created_by, idempotency_key));
    CREATE UNIQUE INDEX administrator_invitations_one_pending_email ON administrator_invitations(email_canonical) WHERE status = 'PENDING';
    CREATE TABLE identity_email_outbox (id uuid PRIMARY KEY, verification_token_id uuid REFERENCES email_verification_tokens(id), administrator_invitation_id uuid REFERENCES administrator_invitations(id), recipient_email text NOT NULL, message_type text NOT NULL, ciphertext text NOT NULL, encrypted_dek text NOT NULL, iv text NOT NULL, tag text NOT NULL, wrap_iv text NOT NULL, wrap_tag text NOT NULL, aad text NOT NULL, key_version integer NOT NULL, status text NOT NULL DEFAULT 'PENDING', available_at timestamptz NOT NULL DEFAULT now(), claimed_by text, claimed_until timestamptz, attempt_count integer NOT NULL DEFAULT 0, max_attempts integer NOT NULL DEFAULT 5, provider_message_id text, last_error_code text, created_at timestamptz NOT NULL DEFAULT now(), sent_at timestamptz);
