...
export async function createMemoryDatabase(): Promise<DeliveryDatabase> {
  ...
  memory.public.none(`
    CREATE TABLE users (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), email text NOT NULL, email_canonical text GENERATED ALWAYS AS (lower(trim(email))) STORED UNIQUE, status text NOT NULL, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now());
    ...
    CREATE TABLE refresh_sessions (id uuid PRIMARY KEY, family_id uuid NOT NULL, user_id uuid NOT NULL REFERENCES users(id), organization_id uuid NOT NULL REFERENCES organizations(id), token_hash text NOT NULL UNIQUE, expires_at timestamptz NOT NULL, used_at timestamptz, revoked_at timestamptz, replaced_by uuid, created_at timestamptz NOT NULL DEFAULT now());
    ...
    CREATE TABLE audit_events (id bigserial PRIMARY KEY, organization_id uuid NOT NULL, project_id uuid, actor_type text NOT NULL, actor_id text NOT NULL, action text NOT NULL, target_type text NOT NULL, target_id text NOT NULL, trace_id text NOT NULL, metadata jsonb NOT NULL DEFAULT '{}', occurred_at timestamptz NOT NULL DEFAULT now());
